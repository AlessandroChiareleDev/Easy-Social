    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-white flex items-center gap-3">
          <svg class="w-7 h-7 text-[#5ac8f5]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <p class="text-sm text-slate-400 mt-1">Visão por CPF — S-1200 (Remuneração) e S-1210 (Pagamentos)</p>
      </div>
    </div>
    ...
    <div class="stats-grid" v-if="stats">
      ...
      <div class="stat-card">
        ...
        <div>
          ...
          <div class="stat-label">Eventos S-1200</div>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div>
          ...
          <div class="stat-label">Eventos S-1210</div>
        </div>
      </div>
      <div class="stat-card" :class="{ 'stat-card--success': (stats.cpfs_retificados ?? 0) > 0 }">
        <div class="stat-icon" :class="(stats.cpfs_retificados ?? 0) > 0 ? 'stat-icon--green' : 'stat-icon--red'">
    ...
    <div class="filter-card">
      <div class="filter-grid">
        <div class="filter-group">
          ...
          <select class="filter-select" v-model="filters.per_apur" @change="buscar(1)">
            <option value="">Todos os períodos</option>
            <option v-for="p in stats?.periodos ?? []" :key="p" :value="p">{{ formatPeriodo(p) }}</option>
          </select>
        </div>
        <div class="filter-group">
    ...
    <div v-else-if="result" class="results-section">
      <div class="results-header">
        <span class="text-sm text-slate-400">
          {{ result.total }} CPF{{ result.total !== 1 ? 's' : '' }} encontrado{{ result.total !== 1 ? 's' : '' }}
        ...
        <table class="results-table">
          ...
          <tbody>
            <template v-for="item in result.items" :key="item.cpf">
              <tr class="result-row" :class="{ 'result-row--expanded': expandedCpf === item.cpf }" @click="toggleExpand(item.cpf)">
                <td>
                  <svg
                ...
                <td class="text-sm text-slate-300">
                  {{ item.vr_liq ? `R$ ${Number(item.vr_liq).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '—' }}
                </td>
                <td>
                  <span
                    :class="['status-badge', item.retificado ? 'status-badge--done' : 'status-badge--pending']"
                  >
                  ...
                  <div class="detail-content">
                    ...
                    <div class="detail-meta">
                      ...
                      <div class="meta-item">
                        ...
                        <span class="meta-value font-mono text-xs">{{ item.nr_recibo_s1210 || '—' }}</span>
                      </div>
                      <div class="meta-item">
                        <span class="meta-label">Último Processamento</span>
                        <span class="meta-value">{{ formatDateTime(item.ultimo_processamento) }}</span>
                      </div>
                      <div class="meta-item" v-if="item.periodos">
                        <span class="meta-label">Períodos</span>
                        <span class="meta-value">{{ (item.periodos || []).map(formatPeriodo).join(', ') }}</span>
                      </div>
                    </div>
                    ...
                    <div v-else-if="detailEvents.length">
                      <h4 class="text-xs font-semibold text-[rgba(90,200,245,0.6)] uppercase tracking-wider mb-3 border-t border-[rgba(90,200,245,0.08)] pt-3">
                        Eventos Detalhados
                      </h4>
                      <div v-for="evt in detailEvents" :key="evt.id" class="detail-event-card">
                        <div class="detail-event-header">
                          <span :class="['event-badge', eventBadgeClass(evt.tipo_evento)]">{{ evt.tipo_evento }}</span>
                          <span class="text-xs text-slate-400">{{ evt.arquivo_origem }}</span>
                          <span v-if="evt.nr_recibo" class="text-xs font-mono text-slate-500">{{ evt.nr_recibo }}</span>
                        </div>
                          ...
                          <table class="rubricas-table">
                            ...
                            <tbody>
                              <tr v-for="r in evt.rubricas" :key="r.id">
                                ...
                                <td>{{ r.nat_rubr || '—' }}</td>
                                <td>
                                  <span v-if="r.cod_inc_irrf === '11'" class="text-red-400 font-bold">{{ r.cod_inc_irrf }}</span>
                                  <span v-else>{{ r.cod_inc_irrf || '—' }}</span>
                                </td>
                                <td class="text-right">{{ r.vr_rubr ? Number(r.vr_rubr).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : '—' }}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
      ...
      <div v-else class="empty-state">
        ...
        <p class="text-xs text-slate-500 mt-2">Importe XMLs na aba "Explorador de Eventos" para popular os dados.</p>
      </div>

      <!-- Pagination -->
      <div v-if="result.total_pages > 1" class="pagination">
        <button class="page-btn" :disabled="result.page <= 1" @click="buscar(result.page - 1)">‹</button>
        <button
        ...
        <button class="page-btn" :disabled="result.page >= result.total_pages" @click="buscar(result.page + 1)">›</button>
      </div>
    </div>
  </div>
</template>
...
.stat-card:hover { border-color: rgba(90, 200, 245, 0.22); box-shadow: 0 0 20px rgba(90, 200, 245, 0.06); }
.stat-card--success { border-color: rgba(16, 185, 129, 0.25) !important; }
.stat-card--success:hover { box-shadow: 0 0 20px rgba(16, 185, 129, 0.1); }
...
.filter-label { font-size: 0.6875rem; font-weight: 600; color: rgba(90, 200, 245, 0.6); text-transform: uppercase; letter-spacing: 0.05em; }
...
.filter-input:focus, .filter-select:focus { border-color: var(--brain-blue); box-shadow: 0 0 12px rgba(90, 200, 245, 0.15); }
.filter-select {
  appearance: none;
...
.filter-actions { display: flex; gap: 10px; justify-content: flex-end; padding-top: 14px; border-top: 1px solid rgba(90, 200, 245, 0.08); }

.btn-search {
  display: flex; align-items: center; gap: 8px; padding: 8px 22px;
  background: rgba(90, 200, 245, 0.15); border: 1px solid rgba(90, 200, 245, 0.3);
  border-radius: 10px; color: var(--brain-blue); font-size: 0.875rem; font-weight: 600;
...
.btn-clear {
  ...
  border-radius: 10px; color: #64748b; font-size: 0.8125rem; cursor: pointer; transition: all 0.25s ease;
}
...
.loading-spinner {
  width: 36px; height: 36px; border: 3px solid rgba(90, 200, 245, 0.15);
  border-top-color: var(--brain-blue); border-radius: 50%;
...
.results-table-wrapper {
  background: var(--glass-bg); backdrop-filter: blur(16px); border-radius: 14px;
...
.results-table th {
  padding: 12px 14px; text-align: left; font-size: 0.6875rem; font-weight: 600;
  color: rgba(90, 200, 245, 0.6); text-transform: uppercase; letter-spacing: 0.05em;
...
.event-badge {
  ...
  font-size: 0.6875rem; font-weight: 700; font-family: monospace; letter-spacing: 0.02em;
}
.badge--blue { background: rgba(59, 130, 246, 0.15); color: #60a5fa; border: 1px solid rgba(59, 130, 246, 0.25); }
.badge--cyan { background: rgba(90, 200, 245, 0.12); color: #5ac8f5; border: 1px solid rgba(90, 200, 245, 0.2); }
.badge--slate { background: rgba(100, 116, 139, 0.12); color: #94a3b8; border: 1px solid rgba(100, 116, 139, 0.2); }
...
/* ── Detail row ──────────────────────────────────── */
.detail-row td { padding: 0 !important; border-bottom: 1px solid rgba(90, 200, 245, 0.08) !important; }
...
.meta-label { font-size: 0.625rem; font-weight: 600; color: rgba(90, 200, 245, 0.5); text-transform: uppercase; letter-spacing: 0.05em; }
.meta-value { color: #cbd5e1; word-break: break-all; }

/* ── Detail event cards ──────────────────────────── */
.detail-event-card {
  background: rgba(0, 0, 0, 0.2); border: 1px solid rgba(90, 200, 245, 0.08);
  border-radius: 10px; padding: 12px 16px; margin-bottom: 10px;
}
.detail-event-header { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; flex-wrap: wrap; }
.detail-event-fields {
...
/* ── Rubricas detail ─────────────────────────────── */
.rubricas-detail { border-top: 1px solid rgba(90, 200, 245, 0.08); padding-top: 10px; margin-top: 8px; }
.rubricas-table { width: 100%; border-collapse: separate; border-spacing: 0; }
.rubricas-table th {
  padding: 6px 10px; font-size: 0.625rem; font-weight: 600;
  color: rgba(90, 200, 245, 0.5); text-transform: uppercase; letter-spacing: 0.05em;
  text-align: left; border-bottom: 1px solid rgba(90, 200, 245, 0.08);
}
.rubricas-table td { padding: 6px 10px; font-size: 0.75rem; border-bottom: 1px solid rgba(255, 255, 255, 0.03); }
...
.page-btn {
  ...
  padding: 0 8px; border-radius: 8px; font-size: 0.8125rem; font-weight: 500;
  color: #94a3b8; border: 1px solid rgba(255, 255, 255, 0.08); background: transparent;
  cursor: pointer; transition: all 0.2s ease;
}
.page-btn:hover:not(:disabled) { color: #fff; border-color: rgba(90, 200, 245, 0.2); }
.page-btn:disabled { opacity: 0.3; cursor: not-allowed; }
.page-btn--active {
  color: var(--brain-blue); border-color: rgba(90, 200, 245, 0.3);
