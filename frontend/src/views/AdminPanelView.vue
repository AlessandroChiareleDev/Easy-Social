    ...
    <div class="relative z-10 px-8 mt-6 pb-16 max-w-7xl mx-auto">
      ...
      <div class="flex gap-1 mb-6">
        ...
        >
          ...
          <span
            ...
            class="ml-1.5 text-xs bg-white/10 px-1.5 py-0.5 rounded-full"
          >
            {{ tab.count }}
          </span>
        </button>
      ...
      <template v-if="activeTab === 'envios'">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6" v-if="enviosResumo">
          <div
            ...
            v-for="s in enviosResumo.por_status"
            :key="s.status"
          >
        ...
        <div class="glass-card p-5">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr
                  ...
                  @click="
                    envioExpandido === e.id ? (envioExpandido = null) : (envioExpandido = e.id)
                  "
                >
                  <td class="py-2 px-3 text-xs text-white/50">
                    {{ formatDateTime(e.created_at) }}
                  </td>
                  <td class="py-2 px-3 font-mono font-medium text-[#0066FF]">
                    {{ e.tipo_evento }}
                  </td>
                  <td class="py-2 px-3">
                    <span
                      :class="e.modo === 'inclusao' ? 'text-green-400' : 'text-amber-400'"
                      class="text-xs"
                    >
                      {{ e.modo }}
                    </span>
                  ...
                  <td
                    class="py-2 px-3 text-xs max-w-[200px] truncate"
                    :class="
                      e.codigo_resposta === '201'
                        ? 'text-green-400'
                        : e.codigo_resposta
                          ? 'text-amber-400'
                          : 'text-white/60'
            ...
            >
              <div
                v-for="e in envios.filter((x) => x.id === envioExpandido)"
                :key="'detail-' + e.id"
              >
                ...
                <div v-if="e.rubrica_detalhes && e.rubrica_detalhes.length > 0" class="mt-3">
                  <h4 class="text-white/60 text-xs mb-2">
                    Rubricas enviadas ({{ e.rubrica_detalhes.length }}):
                  </h4>
                  <div class="flex flex-wrap gap-1">
                    <span
                      v-for="(rb, ri) in e.rubrica_detalhes.slice(0, 20)"
                      :key="ri"
                      class="bg-white/10 px-2 py-0.5 rounded text-xs text-white/70 font-mono"
                    >
                ...
                <div
                  v-if="
                    e.ocorrencias &&
                    (Array.isArray(e.ocorrencias) ? e.ocorrencias.length > 0 : true)
                  "
                  class="mt-3"
                >
                  <h4 class="text-amber-400 text-xs mb-2">Ocorrências:</h4>
                  <pre
                    class="text-xs text-white/60 bg-black/30 p-2 rounded overflow-x-auto max-h-40"
                    >{{ JSON.stringify(e.ocorrencias, null, 2) }}</pre
                  >
                </div>
              </div>
            </div>
          ...
          <!-- Pagination envios -->
          <div
            v-if="totalEnviosPage > pageSize"
            class="flex items-center justify-center gap-4 mt-4"
          >
            <button
              @click="envioPage > 0 && (envioPage--, loadEnvios())"
              :disabled="envioPage === 0"
              class="btn-secondary text-sm"
              :class="{ 'opacity-30 cursor-not-allowed': envioPage === 0 }"
            >
              ← Anterior
            </button>
            <span class="text-white/50 text-sm"
              >Página {{ envioPage + 1 }} de {{ envioTotalPages }}</span
            >
            <button
              @click="envioPage < envioTotalPages - 1 && (envioPage++, loadEnvios())"
              :disabled="envioPage >= envioTotalPages - 1"
              class="btn-secondary text-sm"
              :class="{ 'opacity-30 cursor-not-allowed': envioPage >= envioTotalPages - 1 }"
            >
              Próxima →
            </button>
          </div>
      ...
      <template v-if="activeTab === 'pipeline'">
        ...
        <div class="glass-card p-4 mb-6 flex flex-wrap items-center gap-4">
          ...
          <label class="text-white/60 text-sm">Ambiente:</label>
          <select
            v-model="pipelineFiltroAmbiente"
            @change="loadPipelines"
            class="glass-input text-sm"
          >
            <option value="">Todos</option>
        ...
        <div class="glass-card p-5">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr
                  ...
                  class="text-white/70 border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer"
                  @click="
                    pipelineExpandido === p.id
                      ? (pipelineExpandido = null)
                      : (pipelineExpandido = p.id)
                  "
                >
                  <td class="py-2 px-3 text-xs text-white/50">
                  ...
                  <td class="py-2 px-3">
                    <span
                      :class="pipelineStatusClass(p.status)"
                  ...
                  <td class="py-2 px-3 text-xs text-red-400 max-w-[200px] truncate">
                    {{ p.erro || '' }}
                  </td>
                </tr>
            ...
            <div
              v-if="pipelineExpandido"
              class="mt-4 p-4 bg-white/5 rounded-lg border border-white/10"
            >
              <div
                v-for="p in pipelines.filter((x) => x.id === pipelineExpandido)"
                :key="'pdetail-' + p.id"
              >
                <h3 class="text-white/80 font-semibold mb-3">
                  Pipeline #{{ p.id }} — {{ formatCpf(p.cpf) }} — {{ p.per_apur }}
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-5 gap-3 mb-3">
                  <div
                    v-for="step in ['s1010', 's1298', 's1200', 's1210', 's1299']"
                    :key="step"
                ...
                <div v-if="p.steps_log && p.steps_log.length > 0">
                  ...
                  <pre
                    class="text-xs text-white/60 bg-black/30 p-2 rounded overflow-x-auto max-h-60"
                    >{{ JSON.stringify(p.steps_log, null, 2) }}</pre
                  >
                </div>
                <div v-if="p.erro" class="mt-2">
                  <h4 class="text-red-400 text-xs mb-1">Erro:</h4>
                  <pre class="text-xs text-red-300 bg-red-900/20 p-2 rounded overflow-x-auto">{{
                    p.erro
                  }}</pre>
                </div>
          ...
          <div
            v-if="totalPipelinesPage > pageSize"
            class="flex items-center justify-center gap-4 mt-4"
          >
            <button
              @click="pipelinePage > 0 && (pipelinePage--, loadPipelines())"
              :disabled="pipelinePage === 0"
              class="btn-secondary text-sm"
              :class="{ 'opacity-30 cursor-not-allowed': pipelinePage === 0 }"
            >
              ← Anterior
            </button>
            <span class="text-white/50 text-sm"
              >Página {{ pipelinePage + 1 }} de {{ pipelineTotalPages }}</span
            >
            <button
              @click="pipelinePage < pipelineTotalPages - 1 && (pipelinePage++, loadPipelines())"
              :disabled="pipelinePage >= pipelineTotalPages - 1"
              class="btn-secondary text-sm"
              :class="{ 'opacity-30 cursor-not-allowed': pipelinePage >= pipelineTotalPages - 1 }"
            >
              Próxima →
            </button>
          </div>
