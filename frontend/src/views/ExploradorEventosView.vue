            ...
            <div v-if="!importing && !importResult">
              ...
              >
                <input
                  ...
                  @change="onFileSelect"
                />
                <div v-if="uploadFiles.length === 0" class="drop-zone-empty" @click="fileInputRef?.click()">
                  <svg class="w-12 h-12 text-slate-500 mx-auto mb-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                    <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" />
                    <polyline points="17 8 12 3 7 8" />
                    <line x1="12" y1="3" x2="12" y2="15" />
                  </svg>
                ...
                <div v-else class="drop-zone-loaded">
                  <div class="flex items-center justify-between mb-2">
                    ...
                    <button @click.stop="clearUploadFiles" class="text-slate-500 hover:text-red-400 text-xs">Limpar</button>
                  </div>
                  <p class="text-sm text-slate-400">
...
async function onDrop(e: DragEvent) {
  ...
  if (files.length === 0) { alert('Nenhum arquivo XML encontrado na pasta'); return }
...
function onFileSelect(e: Event) {
  ...
  if (files.length === 0) { alert('Nenhum arquivo XML encontrado'); return }
  uploadFiles.value = files
  uploadTotalSize.value = files.reduce((sum, f) => sum + f.size, 0)
  detectPeriodFromFilenames(files)
        ...
        if (res.ok) {
          ...
          if (prog.total > 0) {
            ...
            if (prog.rate > 0) {
              ...
              importProgressEta.value = etaSec < 60 ? `~${etaSec}s restantes` : `~${Math.round(etaSec / 60)}min restantes`
            }
          }
