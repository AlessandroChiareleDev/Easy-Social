"""
Script de envio S-1010 ALTERAÇÃO para rubrica 156 (DIF. FERIAS) em PRODUÇÃO
A rubrica existe em produção, então usamos o modo ALTERAÇÃO.
tpAmb=1 (Produção), URLs de produção (webservices.esocial.gov.br)
...
def salvar_recibo(nome: str, conteudo: str):
    """Salva um arquivo no repositório de recibos."""
...
def main():
    ...
    print("Modo: ALTERAÇÃO (rubrica existe em produção)")
    print("URL: webservices.esocial.gov.br")
    print("=" * 70)
    ...
    # 2. Gerar XML ALTERAÇÃO com tpAmb=1 (PRODUÇÃO)
    xml_bytes = S1010XMLGenerator.gerar_alteracao(empregador, RUBRICA_156, tp_amb="1")
    print(f"[2] XML alteração gerado: {len(xml_bytes)} bytes (tpAmb=1 PRODUÇÃO)")
    ...
    # Salvar XML gerado no repositório de recibos
    salvar_recibo(f"rubrica_156_producao_xml_gerado_{ts}.xml", xml_pretty)
    ...
    salvar_recibo(f"rubrica_156_producao_xml_assinado_{ts}.xml", xml_assinado_str)
    ...
    # Salvar SOAP enviado
    salvar_recibo(f"rubrica_156_producao_soap_enviado_{ts}.xml", soap)
    ...
    # Salvar resultado do envio
    salvar_recibo(f"rubrica_156_producao_resultado_envio_{ts}.json",
    ...
    if protocolo:
        ...
        # Salvar resultado da consulta
        salvar_recibo(f"rubrica_156_producao_resultado_consulta_{ts}.json",
    ...
    print("\n" + "=" * 70)
    print("ENVIO PRODUÇÃO CONCLUÍDO")
