"""
Envio Lote 2 Dezembro/2025 APPA (empresa_id=1).

Fluxo:
1. Le CPFs de s1210_cpf_scope (populado por ingest_lote2_dezembro.py).
...
5. Salva JSON de resposta em saida_lote2_dezembro/resp_<ts>.json.
6. Parada automatica em timeout de rede ou erro HTTP != 2xx.

Uso:
  # Dry run — so mostra CPFs que seriam enviados
  python envio_lote2_dezembro.py --dry-run

  # Rodar 1 CPF (teste inicial — SEMPRE comecar aqui)
  python envio_lote2_dezembro.py --max 1

  # Rodar primeiros 10
  python envio_lote2_dezembro.py --max 10

  # Rodar tudo (batches de 50)
  python envio_lote2_dezembro.py

  # Override de recibo via XLSX da Ana
  python envio_lote2_dezembro.py --recibos-xlsx "C:\\...\\Recibos.xlsx" --aba Dez_2025 --col-recibo 2
"""
...
BATCH_SIZE = 50
TIMEOUT    = 600
OUTDIR     = os.path.join(ROOT, "saida_lote2_dezembro")
...
def carregar_plan_saude_db() -> dict[str, list[dict]]:
    ...
    1 entrada por CNPJ — rubricas diferentes do mesmo CNPJ são SOMADAS.
    eSocial não aceita dois <planSaude> com o mesmo cnpjOper."""
    sql = """
        SELECT cpf,
               cnpj_operadora,
...
def carregar_recibos_override_db() -> dict[str, str]:
    ...
    Cobre o caso de lote 1 ja ter retificado esse CPF — recibo do ZIP estaria desatualizado.
    """
    sql = """
...
def main() -> None:
    ...
    # Recibos override: DB primeiro, XLSX sobrescreve se informado
    recibos_override: dict[str, str] = {}
    ...
    if args.recibos_xlsx:
        ...
        xlsx_over = carregar_recibos_override(
            args.recibos_xlsx, args.aba, args.col_cpf, args.col_recibo
        )
        print(f"[override XLSX] {len(xlsx_over)} recibos de {args.recibos_xlsx}")
        recibos_override.update(xlsx_over)

    # planSaude do DB (ingest ja populou s1210_operadoras)
    ...
    if not plan_saude_override:
        print("[AVISO] nenhum planSaude encontrado — verifique se ingest foi rodado")
