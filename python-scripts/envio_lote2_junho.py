"""Import + Envio Lote 2 Junho/2025 APPA.
...
Uso:
  python envio_lote2_junho.py --dry-run   # apenas analisa, sem gravar
  python envio_lote2_junho.py             # import + envio real
"""
...
# ── Configuração ──────────────────────────────────────────────────────────
XLSX_PATH = Path(r"C:\Users\xandao\Downloads\06_Junho_lote 002_APPA.xlsx")
PER_APUR  = "2025-06"
LOTE_NUM  = 2
EMPRESA_ID = 1
...
BATCH_SIZE = 50
TIMEOUT    = 600
OUTDIR     = Path(ROOT) / "saida_lote2_junho"
...
def parse_xlsx():
    ...
    # ── Aba operadoras (layout Junho: 91 colunas) ──
    # col8=CPF, col10=cod, col11=nome, col14=CNPJ, col15=ANS, col18=ValorEvento
    ws2 = wb[ABA_OPER]
    oper_rows: list[dict] = []
    for i, row in enumerate(ws2.iter_rows(values_only=True), 1):
        ...
        cpf = clean_cpf(row[8])
        if not cpf: continue
        cnpj = str(row[14] or "").strip()
        ans  = str(row[15] or "").strip()
        vlr  = row[18]
        cod  = row[10]
        nome_ev = str(row[11] or "")
        # Ignora informativos / #N/A
        if cnpj.lower() in ("informativo", "", "none", "nan", "#n/a"): continue
        if ans.lower() in ("informativo", "", "none", "nan", "#n/a"): continue
        cnpj_d = re.sub(r"\D", "", cnpj)
        ans_d  = re.sub(r"\D", "", ans)
        if len(cnpj_d) != 14 or not ans_d: continue
        try:
...
def main():
    ...
    print(f"=== Envio Junho 2025 L2 — {XLSX_PATH.name} ===\n")
