"""
Envio Lote 2 Novembro/2025 APPA (empresa_id=1).

Fluxo:
1. Le CPFs de s1210_cpf_scope (populado por ingest_lote2_novembro.py).
...
5. Salva JSON de resposta em saida_lote2_novembro/resp_<ts>.json.
6. Parada automatica em timeout de rede ou erro HTTP != 2xx.

Uso:
  # Dry run — so mostra CPFs que seriam enviados
  python envio_lote2_novembro.py --dry-run

  # Rodar 1 CPF (teste inicial — SEMPRE comecar aqui)
  python envio_lote2_novembro.py --max 1

  # Rodar primeiros 10
  python envio_lote2_novembro.py --max 10

  # Rodar tudo (batches de 50, sequencial)
  python envio_lote2_novembro.py

  # Rodar com workers=3 (mais rapido, pode gerar 1089)
  python envio_lote2_novembro.py --workers 3
"""
...
BATCH_SIZE = 50
TIMEOUT    = 600
OUTDIR     = os.path.join(ROOT, "saida_lote2_novembro")
...
def main() -> None:
    ...
    # Recibos override do DB (lote 1 de novembro pode ter gerado recibos novos)
