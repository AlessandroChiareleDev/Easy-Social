"""
Envio Lote 3 Dezembro/2025 APPA (empresa_id=1) â€” receita PC1-15-v2.
...
Uso:
  # Dry run â€” so mostra CPFs que seriam enviados, nao envia
...
BATCH_SIZE = 50
TIMEOUT = 600
OUTDIR = os.path.join(ROOT, "saida_lote3_dezembro")
...
def carregar_plan_saude_db() -> dict[str, list[dict]]:
    ...
    1 entrada por CNPJ; soma valores no mesmo CNPJ; valor em REAIS (xx.yy)."""
    sql = """
        SELECT cpf,
               cnpj_operadora,
...
def carregar_plan_saude_xlsx(xlsx: str, aba: str = "Assistencia Medica") -> dict[str, list[dict]]:
    ...
    # aba normalizada â€” pegar 'Assistencia/Assitencia Medica' tolerante a typo/acento
    import unicodedata
    def _norm(s: str) -> str:
...
def main() -> None:
    ...
    # 1) DB: pega recibo_novo do ultimo envio OK por CPF (cobre Lote 1 Maio jÃ¡ retificado)
    ...
    if args.plan_saude_xlsx:
        ...
        # Default: carrega direto de s1210_operadoras (ingest XLSX ja populou)
    ...
    if args.workers > 1:
        ...
        for idx, bloco in enumerate(batches, 1):
            print(f"\n=== bloco {idx}/{len(batches)} â€” {len(bloco)} CPFs ===")
            try:
                body = enviar_bloco(bloco, recibos_override, plan_saude_override)
            except requests.exceptions.RequestException as e:
