...
OUT_XLSX = OUT_DIR / "2025-04_relatorio_final_jaque.xlsx"
RESPONSE_XLSX = ROOT / "relatorio_ana" / "RESPOSTAS_JAQUE_PLANO_PENSAO_2025_FEV_OUT" / "SOLUCOES_ABRIL_2025_JAQUE_PLANO_PENSAO_PREENCHER.xlsx"
...
def build_summary(ws, counts: Counter[str]) -> None:
    add_title(ws, "Relatório final Jaque - Abril/2025", "Pendências finais após as correções possíveis do mês.")
    ws.merge_cells("A4:F8")
    msg = ws["A4"]
    msg.value = (
        "Abril ficou com 14 pendências atuais no S-1210. Foram resolvidas as respostas válidas de plano/pensão "
        "e os casos de dependente 1861 que permitiam correção por infoDep. Restaram 10 planos de saúde sem dados "
        "válidos na resposta da Jaque e 4 recibos sem recibo ativo confiável localizado nos dados locais."
    )
    msg.fill = SOFT_FILL
    ...
    rows = [
        ...
        ["Recibo", counts["recibo"], "Precisa recibo ativo do S-1210 de abril para o mesmo CPF/período/objeto."],
    ]
    write_table(ws, 11, ["Tipo", "Qtd. trabalhadores", "Próxima ação"], rows, "AbrilResumo")
...
def build_plan(ws, plan_rows: dict[str, dict[str, Any]], latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Plano de saúde - Abril/2025")
    ...
    write_table(ws, 3, headers, rows, "PlanoSaudeAbril")
    add_note(ws, "O eSocial recusou porque o grupo de plano de saúde coletivo não pôde ser montado. Na resposta de abril, o CNPJ veio como texto 'ok' e Registro ANS/valor titular ficaram vazios para estes CPFs.", 10)
    autosize(ws, {1: 18, 2: 17, 3: 34, 4: 20, 5: 18, 6: 24, 7: 48, 8: 52})


def build_receipt(ws, latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Recibo - Abril/2025")
    ...
    for cpf in sorted(latest):
        ...
        rows.append([
            ...
            "Enviar recibo ativo do mesmo S-1210, mesmo CPF, mesmo período 2025-04 e mesmo objeto/demonstrativo.",
            short_error(str(item.get("erro_mensagem") or ""), 220),
        ])
    write_table(ws, 3, headers, rows, "ReciboAbril")
    add_note(ws, "Não há recibo ativo confiável para estes quatro CPFs nos dados locais verificados. pipeline_cpf_results não possui linhas para eles; os ZIPs locais de abril só trouxeram S-1210 de perApur 2025-03 para dois CPFs e nada útil para perApur 2025-04.", 7)
...
def build_workbook() -> dict[str, Any]:
    ...
    build_summary(workbook.create_sheet("Abril"), counts)
