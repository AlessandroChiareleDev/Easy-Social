...
OUT_XLSX = OUT_DIR / "2025-07_relatorio_final_jaque.xlsx"
RESPONSE_XLSX = ROOT / "relatorio_ana" / "RESPOSTAS_JAQUE_PLANO_PENSAO_2025_FEV_OUT" / "SOLUCOES_JULHO_2025_JAQUE_PLANO_PENSAO_PREENCHER.xlsx"
...
def latest_counts() -> dict[str, Any]:
    internal_empresa_id = tenant.internal_empresa_id(EMPRESA_ID)
    conn = db.connect(empresa_id=EMPRESA_ID)
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
...
def build_summary(ws, counts: Counter[str], status_counts: dict[str, int]) -> None:
    add_title(ws, "Relatório final Jaque - Julho/2025", "Pendências finais após parar as tentativas deste mês.")
    ws.merge_cells("A4:F9")
    msg = ws["A4"]
    msg.value = (
        "Julho ficou com 5 pendências atuais no S-1210. As respostas válidas de plano/pensão foram reenviadas, "
        "os dependentes 1861 corrigíveis foram aceitos e o caso de identificador duplicado 543 foi reenviado com novo Id. "
        "Por orientação operacional, o restante foi colocado neste relatório: 3 planos de saúde sem dados válidos suficientes "
        "e 2 recibos recusados pelo eSocial."
    )
    msg.fill = SOFT_FILL
    ...
    rows = [
        ["Plano de saúde", counts["plano"], "Jaque precisa reenviar Registro ANS e valor titular; conferir CNPJ operadora."],
        ["Recibo", counts["recibo"], "Precisa recibo ativo do S-1210 de julho para o mesmo CPF/período/objeto."],
    ]
    write_table(ws, 12, ["Tipo", "Qtd. trabalhadores", "Próxima ação"], rows, "JulhoResumo")
    status_rows = [[key, value] for key, value in sorted(status_counts.items())]
    write_table(ws, 17, ["Status latest S-1210", "Qtd."], status_rows, "JulhoStatusLatest")
    ws["A24"] = f"Gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')}"
    ws["A24"].font = Font(italic=True, color="666666")
    autosize(ws, {1: 28, 2: 18, 3: 76})


def build_plan(ws, plan_rows: dict[str, dict[str, Any]], latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Plano de saúde - Julho/2025")
    headers = [
        "CPF",
        "CPF Normalizado",
        "Nome Trabalhador",
    ...
    for cpf in sorted(latest):
        ...
        rows.append(
            [
                ...
                "Informar Registro ANS e valor titular descontado em folha; conferir CNPJ operadora numérico.",
                short_error(str(item.get("erro_mensagem") or ""), 180),
            ]
        )
    write_table(ws, 3, headers, rows, "PlanoSaudeJulho")
    add_note(
        ws,
        "O eSocial recusou porque o grupo de plano de saúde coletivo não pôde ser montado. Para estes CPFs, a resposta de julho não tem Registro ANS e/ou valor titular aproveitável para montar uma retificação segura.",
        10,
    )
...
def build_receipt(ws, latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Recibo - Julho/2025")
    ...
    for cpf in sorted(latest):
        ...
        rows.append(
            [
                ...
                "Enviar recibo ativo do mesmo S-1210, mesmo CPF, mesmo período 2025-07 e mesmo objeto/demonstrativo.",
                short_error(str(item.get("erro_mensagem") or ""), 230),
            ]
        )
    write_table(ws, 3, headers, rows, "ReciboJulho")
    add_note(
        ws,
        "O eSocial recusou o recibo usado na retificação. Sem um recibo ativo confiável para o mesmo S-1210 de julho, não é seguro reenviar tentando adivinhar outro recibo.",
        7,
    )
...
def build_workbook() -> dict[str, Any]:
    ...
    by_type: dict[str, dict[str, dict[str, Any]]] = {"plano": {}, "recibo": {}, "outro": {}}
    for item in errors:
        kind = error_type(str(item.get("erro_mensagem") or ""))
        by_type[kind][str(item["cpf"])] = item
    if by_type["outro"]:
        raise RuntimeError(f"Pendências não classificadas para julho: {sorted(by_type['outro'])}")

    plan_data = response_rows(set(by_type["plano"]))
    counts = Counter({"plano": len(by_type["plano"]), "recibo": len(by_type["recibo"])})
    workbook = Workbook()
    workbook.remove(workbook.active)
    build_summary(workbook.create_sheet("Julho"), counts, latest_counts())
