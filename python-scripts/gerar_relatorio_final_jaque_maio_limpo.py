...
OUT_XLSX = OUT_DIR / "2025-05_relatorio_final_jaque.xlsx"
RESPONSE_XLSX = ROOT / "relatorio_ana" / "RESPOSTAS_JAQUE_PLANO_PENSAO_2025_FEV_OUT" / "SOLUCOES_MAIO_2025_JAQUE_PLANO_PENSAO_PREENCHER.xlsx"
...
def build_summary(ws, counts: Counter[str]) -> None:
    add_title(ws, "Relatório final Jaque - Maio/2025", "Pendências finais após as correções possíveis do mês.")
    ws.merge_cells("A4:F8")
    msg = ws["A4"]
    msg.value = (
        "Maio ficou com 3 pendências atuais no S-1210. Foram resolvidas as respostas válidas de plano/pensão, "
        "os dependentes 1861 e os casos de plano 1867 com valor titular zerado quando havia valor positivo na aba "
        "plano saude - ok. Restaram 2 planos de saúde sem dados válidos na resposta da Jaque e 1 recibo sem recibo ativo confiável localizado nos dados locais."
    )
    msg.fill = SOFT_FILL
    ...
    rows = [
        ...
        ["Recibo", counts["recibo"], "Precisa recibo ativo do S-1210 de maio para o mesmo CPF/período/objeto."],
    ]
    write_table(ws, 11, ["Tipo", "Qtd. trabalhadores", "Próxima ação"], rows, "MaioResumo")
...
def build_plan(ws, plan_rows: dict[str, dict[str, Any]], latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Plano de saúde - Maio/2025")
    ...
    write_table(ws, 3, headers, rows, "PlanoSaudeMaio")
    add_note(ws, "O eSocial recusou porque o grupo de plano de saúde coletivo não pôde ser montado. Na resposta de maio, estes CPFs aparecem como 'OK', mas sem CNPJ operadora, Registro ANS e valor titular aproveitáveis.", 10)
    autosize(ws, {1: 18, 2: 17, 3: 34, 4: 20, 5: 18, 6: 24, 7: 48, 8: 52})


def build_receipt(ws, latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Recibo - Maio/2025")
    ...
    for cpf in sorted(latest):
        ...
        rows.append([
            ...
            "Enviar recibo ativo do mesmo S-1210, mesmo CPF, mesmo período 2025-05 e mesmo objeto/demonstrativo.",
            short_error(str(item.get("erro_mensagem") or ""), 220),
        ])
    write_table(ws, 3, headers, rows, "ReciboMaio")
    add_note(ws, "Não há recibo ativo confiável para este CPF nos dados locais verificados. pipeline_cpf_results não possui linha útil; os ZIPs locais de maio/junho mostram o mesmo S-1210 com o recibo recusado pelo eSocial e não mostram substituto ativo seguro para retificação.", 7)
...
def build_workbook() -> dict[str, Any]:
    ...
    build_summary(workbook.create_sheet("Maio"), counts)
