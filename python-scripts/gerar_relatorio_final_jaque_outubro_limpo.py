...
OUT_XLSX = OUT_DIR / "2025-10_relatorio_final_jaque.xlsx"
RESPONSE_XLSX = ROOT / "relatorio_ana" / "RESPOSTAS_JAQUE_PLANO_PENSAO_2025_FEV_OUT" / "SOLUCOES_OUTUBRO_2025_JAQUE_PLANO_PENSAO_PREENCHER.xlsx"
...
def build_summary(ws, counts: Counter[str], status_counts: dict[str, int]) -> None:
    add_title(ws, "Relatório final Jaque - Outubro/2025", "Pendências finais após as correções possíveis do mês.")
    ws.merge_cells("A4:F8")
    msg = ws["A4"]
    msg.value = (
        "Outubro ficou com 3 pendências atuais no S-1210. Foram reenviadas as respostas válidas de plano/pensão "
        "e os dependentes 1861 corrigíveis foram aceitos. Restaram somente planos de saúde sem dados válidos suficientes na resposta da Jaque."
    )
    msg.fill = SOFT_FILL
    ...
    rows = [["Plano de saúde", counts["plano"], "Jaque precisa reenviar CNPJ operadora, Registro ANS e valor titular."]]
    write_table(ws, 11, ["Tipo", "Qtd. trabalhadores", "Próxima ação"], rows, "OutubroResumo")
    status_rows = [[key, value] for key, value in sorted(status_counts.items())]
    write_table(ws, 16, ["Status latest S-1210", "Qtd."], status_rows, "OutubroStatusLatest")
    ws["A23"] = f"Gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')}"
...
def build_plan(ws, plan_rows: dict[str, dict[str, Any]], latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Plano de saúde - Outubro/2025")
    ...
    write_table(ws, 3, headers, rows, "PlanoSaudeOutubro")
    add_note(ws, "O eSocial recusou porque o grupo de plano de saúde coletivo não pôde ser montado. Na resposta de outubro, estes CPFs não têm os dados completos aproveitáveis para uma retificação segura.", 10)
    autosize(ws, {1: 18, 2: 17, 3: 34, 4: 20, 5: 18, 6: 24, 7: 52, 8: 52})


def build_workbook() -> dict[str, Any]:
    errors = latest_errors()
    by_type: dict[str, dict[str, dict[str, Any]]] = {"plano": {}, "outro": {}}
    for item in errors:
        kind = error_type(str(item.get("erro_mensagem") or ""))
        by_type[kind][str(item["cpf"])] = item
    if by_type["outro"]:
        raise RuntimeError(f"Pendências não classificadas para outubro: {sorted(by_type['outro'])}")

    plan_data = response_rows(set(by_type["plano"]))
    counts = Counter({"plano": len(by_type["plano"])})
    workbook = Workbook()
    workbook.remove(workbook.active)
    build_summary(workbook.create_sheet("Outubro"), counts, latest_counts())
