...
OUT_XLSX = OUT_DIR / "2025-09_relatorio_final_jaque.xlsx"
RESPONSE_XLSX = ROOT / "relatorio_ana" / "RESPOSTAS_JAQUE_PLANO_PENSAO_2025_FEV_OUT" / "SOLUCOES_SETEMBRO_2025_JAQUE_PLANO_PENSAO_PREENCHER.xlsx"
...
def error_type(message: str) -> str:
    ...
    if "plano de saúde coletivo" in lowered or "plano de saude coletivo" in lowered:
        return "plano"
    if "vlrdedpenalim" in lowered or "valor '0.00'" in lowered or "value '0.00'" in lowered:
        return "pensao"
    if "459" in lowered or "recibo de entrega informado" in lowered:
        return "recibo"
    return "outro"
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            cursor.execute(
                ...
                WITH latest AS (
                    SELECT DISTINCT ON (it.cpf)
                           it.cpf, it.status, COALESCE(it.erro_codigo, '') AS erro_codigo
                      FROM timeline_envio_item it
                      JOIN timeline_envio te ON te.id = it.timeline_envio_id
                      JOIN timeline_mes tm ON tm.id = te.timeline_mes_id
                     WHERE tm.empresa_id = %s
                ...
                SELECT status, erro_codigo, COUNT(*) AS total
                  FROM latest
        ...
        conn.close()
    return {f"{row['status']}|{row['erro_codigo']}": int(row["total"]) for row in rows}
...
def build_summary(ws, counts: Counter[str], status_counts: dict[str, int]) -> None:
    add_title(ws, "Relatório final Jaque - Setembro/2025", "Pendências finais após as correções possíveis do mês.")
    ws.merge_cells("A4:F9")
    msg = ws["A4"]
    msg.value = (
        "Setembro ficou com 7 pendências atuais no S-1210. Foram reenviadas as respostas válidas de plano/pensão "
        "e os dependentes 1861 corrigíveis foram aceitos. Conforme combinado, não foi feita investigação longa de recibo: "
        "o recibo 459 restante foi colocado no relatório para conferência manual."
    )
    msg.fill = SOFT_FILL
    ...
    rows = [
        ...
        ["Pensão alimentícia", counts["pensao"], "Jaque precisa corrigir valor deduzido: o eSocial não aceita 0,00."],
        ["Recibo", counts["recibo"], "Precisa recibo ativo do S-1210 de setembro para o mesmo CPF/período/objeto."],
    ]
    write_table(ws, 12, ["Tipo", "Qtd. trabalhadores", "Próxima ação"], rows, "SetembroResumo")
    status_rows = [[key, value] for key, value in sorted(status_counts.items())]
    write_table(ws, 18, ["Status latest S-1210", "Qtd."], status_rows, "SetembroStatusLatest")
    ws["A25"] = f"Gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')}"
    ws["A25"].font = Font(italic=True, color="666666")
    autosize(ws, {1: 28, 2: 18, 3: 78})


def build_plan(ws, plan_rows: dict[str, dict[str, Any]], latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Plano de saúde - Setembro/2025")
    ...
    write_table(ws, 3, headers, rows, "PlanoSaudeSetembro")
    add_note(ws, "O eSocial recusou porque o grupo de plano de saúde coletivo não pôde ser montado. Na resposta de setembro, estes CPFs aparecem com CNPJ como texto 'ok' e sem Registro ANS/valor titular aproveitáveis.", 10)
    autosize(ws, {1: 18, 2: 17, 3: 34, 4: 20, 5: 18, 6: 24, 7: 52, 8: 52})


def build_pension(ws, pension_rows: dict[str, dict[str, Any]], latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Pensão alimentícia - Setembro/2025")
    headers = ["CPF", "CPF Normalizado", "Nome Trabalhador", "CPF Beneficiário 1", "Tipo Rendimento 1", "Percentual 1", "Valor Deduzido 1", "O que falta corrigir", "Erro eSocial"]
    rows = []
    for cpf in sorted(latest):
        data = pension_rows.get(cpf, {})
        item = latest[cpf]
        rows.append([
            data.get("CPF") or cpf_mask(cpf),
            cpf,
            clean_text(data.get("Nome Trabalhador")),
            data.get("CPF Beneficiario 1"),
            data.get("Tipo Rendimento 1"),
            data.get("Percentual 1"),
            data.get("Valor Deduzido 1"),
            "Corrigir o valor deduzido: enviar valor positivo ou remover a pensão se não houve desconto.",
            short_error(str(item.get("erro_mensagem") or ""), 220),
        ])
    write_table(ws, 3, headers, rows, "PensaoSetembro")
    add_note(ws, "O XML foi recusado no schema porque a resposta trouxe valor deduzido 0,00. O eSocial exige valor maior que zero para vlrDedPenAlim.", 11)
    autosize(ws, {1: 18, 2: 17, 3: 34, 4: 22, 5: 18, 6: 16, 7: 18, 8: 56, 9: 62})


def build_receipt(ws, latest: dict[str, dict[str, Any]]) -> None:
    add_title(ws, "Recibo - Setembro/2025")
    ...
    for cpf in sorted(latest):
        ...
        rows.append([
            ...
            "Enviar recibo ativo do mesmo S-1210, mesmo CPF, mesmo período 2025-09 e mesmo objeto/demonstrativo.",
            short_error(str(item.get("erro_mensagem") or ""), 230),
        ])
    write_table(ws, 3, headers, rows, "ReciboSetembro")
    add_note(ws, "O eSocial recusou o recibo usado na retificação. Por orientação operacional, não foi feita busca longa de recibo; este item fica para conferência manual.", 7)
    autosize(ws, {1: 18, 2: 28, 3: 28, 4: 58, 5: 62})


def build_workbook() -> dict[str, Any]:
    errors = latest_errors()
    by_type: dict[str, dict[str, dict[str, Any]]] = {"plano": {}, "pensao": {}, "recibo": {}, "outro": {}}
    for item in errors:
        kind = error_type(str(item.get("erro_mensagem") or ""))
        by_type[kind][str(item["cpf"])] = item
    if by_type["outro"]:
        raise RuntimeError(f"Pendências não classificadas para setembro: {sorted(by_type['outro'])}")

    plan_data = response_rows(set(by_type["plano"]))
    pension_data = response_rows(set(by_type["pensao"]))
    counts = Counter({"plano": len(by_type["plano"]), "pensao": len(by_type["pensao"]), "recibo": len(by_type["recibo"])})
    workbook = Workbook()
    workbook.remove(workbook.active)
    build_summary(workbook.create_sheet("Setembro"), counts, latest_counts())
    build_plan(workbook.create_sheet("Plano de saude"), plan_data, by_type["plano"])
    build_pension(workbook.create_sheet("Pensao alimenticia"), pension_data, by_type["pensao"])
    ...
    return {
        ...
        "pensao_linhas_resposta": len(pension_data),
        "status_latest": latest_counts(),
    }
