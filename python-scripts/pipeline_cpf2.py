"""
Pipeline CPF 09820037735 — Audit PRÉ → Correção (sem S-1200) → Audit PÓS

Fluxo simplificado (3 steps de envio):
  1. S-1298 — Reabrir 2024-12
  2. S-1210 retif — Retificar pagamento (forçar recálculo S-5002)
...
# S-1210 dados — último recibo: 1.1.0000000030328699934
...
S1210_INFO_PGTOS = [
    {
        ...
        "tpPgto": "1",
        "perRef": "2024-11",
        "ideDmDev": "20241129.1.00007514",
        "vrLiq": "2198",
    }
]
# Sem info_ir_complem para este CPF
...
def audit_snapshot(tipo):
    ...
    resp = requests.post(f"{API_BASE}/api/pipeline-audit/capturar", json={
        ...
        "per_apur": PER_APUR,
        "tipo": tipo,
        "descricao": f"Snapshot {tipo} — pipeline CPF {CPF}",
        "rubrica_ids": [],
    }, timeout=30)
    data = resp.json()
    if data.get("sucesso"):
...
def main():
    ...
    print(f"  Fluxo: Audit PRÉ → S-1298 → S-1210 retif → S-1299 → Audit PÓS")
    ...
    # ════════════════════════════════════════════
    # STEP 2: S-1210 retif — Retificar pagamento
    # ════════════════════════════════════════════
    print(f"\n{'─'*60}")
    print(f"  STEP 2: S-1210 Retificar pagamento {PER_APUR}")
    ...
    # Salvar resultado
    with open(f"/tmp/pipeline_{CPF}_result.json", "w") as f:
        json.dump({
            "cpf": CPF, "per_apur": PER_APUR, "status": status,
            "steps": [{"nome": n, "result": r} for n, r in results]
        }, f, indent=2, default=str)
    print(f"\nResultado salvo em /tmp/pipeline_{CPF}_result.json")
