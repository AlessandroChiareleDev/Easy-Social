"""
Pipeline CPF 09820037735 — 2025-01 — Correção S-1210

Fluxo:
  1. S-1298 — Reabrir 2025-01
  2. S-1210 retif — Retificar pagamento (2 infoPgtos, dmDevs do XML original)
  3. S-1299 — Fechar 2025-01

ZERO consultas (dados extraídos do XML baixado manualmente).
"""
...
# S-1210 dados — extraídos do XML original (eSocial_EventoRecibo_30913714287.xml)
S1210_NR_RECIBO = "1.1.0000000030913714287"
S1210_INFO_PGTOS = [
    {
        "dtPgto": "2025-01-07",
        "tpPgto": "1",
...
def audit_snapshot(tipo):
    ...
    resp = requests.post(f"{API_BASE}/api/pipeline-audit/capturar", json={
        ...
        "per_apur": PER_APUR,
        "tipo": tipo,
        "descricao": f"Snapshot {tipo} — pipeline CPF {CPF} 2025-01",
        "rubrica_ids": [],
    }, timeout=30)
    data = resp.json()
    if data.get("sucesso"):
...
def main():
    print("=" * 60)
    print(f"  PIPELINE CPF {CPF} — {PER_APUR} — PRODUÇÃO")
    print(f"  Fluxo: S-1298 → S-1210 retif → S-1299")
    print(f"  SEM S-1200 | ZERO consultas")
    print(f"  dmDevs: 01512719 (vrLiq=6), 01512703 (vrLiq=2193)")
    print("=" * 60)
    ...
    try:
        ...
        if not result["sucesso"]:
            print("\n  ATENÇÃO: S-1210 retif falhou! Continuando com S-1299...")
    except Exception as e:
        print(f"  ✗ ERRO: {e}")
    ...
    with open(f"/tmp/pipeline_{CPF}_2025-01_result.json", "w") as f:
        json.dump({
            "cpf": CPF, "per_apur": PER_APUR, "status": status,
            "steps": [{"nome": n, "result": r} for n, r in results]
        }, f, indent=2, default=str)
    print(f"\nResultado salvo em /tmp/pipeline_{CPF}_2025-01_result.json")
