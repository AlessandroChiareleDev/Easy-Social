"""
Reenvio S-1210 — 87 CPFs com erro [1955] (IR negativo).

O problema era no S-1010:  rubricas 571 e 572 tinham codIncIRRF errado.
User corrigiu: 571→33, 572→32. Agora eSocial deve aceitar.

Fluxo:
  1. S-1298   — Reabrir período 2025-09
  2. S-1210   — Retificar os 87 CPFs (mesmos dados, S-1010 já corrigido)
  3. S-1299   — Fechar período 2025-09

Roda na VPS: python3 reenvio_erro1955.py
"""
...
LOG_FILE = f"/tmp/reenvio_erro1955_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
log = logging.getLogger("reenvio_1955")
...
def _load_erro1955_cpfs() -> list[dict]:
    """
    Carrega dados S-1210 para CPFs com erro [1955].
    Usa o recibo que o pipeline já tinha (ainda ativo, retif foi rejeitada).
    """
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute("""
                ...
                ORDER BY cpf
            """)
            erro_cpfs = cur.fetchall()
            cpf_list = [r["cpf"] for r in erro_cpfs]
            recibo_map = {r["cpf"]: r["nr_recibo_original"] for r in erro_cpfs}
            log.info(f"  {len(cpf_list)} CPFs com erro [1955]")

            if not cpf_list:
                return []

            # Buscar dados S-1210 do explorador (usando o recibo que o pipeline usou)
            ...
            # Map: for each CPF, find the event matching the pipeline's recibo
            ...
            result = []
            for cpf in cpf_list:
                target_recibo = recibo_map[cpf]
                events = events_by_cpf.get(cpf, [])
                
                # Find the event matching our recibo
                matched = None
                for evt in events:
                    if evt["nr_recibo"] == target_recibo:
                        matched = evt
                        break
                ...
                if not matched:
                    ...
                    if events:
                        ...
                        log.warning(f"  CPF {cpf}: nenhum evento S-1210 encontrado!")
                        continue

                dados = matched["dados_json"]
                ...
                result.append({
                    "cpf": cpf,
                    "nr_recibo": target_recibo,
                    "pagamentos": pagamentos,
                    "infoIRCR": info_ir_cr,
                })

            log.info(f"  {len(result)} CPFs com dados S-1210")
            return result
    finally:
        conn.close()
...
def main():
    log.info("=" * 70)
    log.info("  REENVIO ERRO [1955] — 87 CPFs (S-1010 corrigido)")
            ...
            with conn.cursor() as cur:
                for cpf in cpfs_ok:
                    cur.execute("""
                        ...
                        WHERE cpf = %s AND status = 'erro' AND erro_descricao LIKE '%%1955%%'
                    """, (cpf,))
    ...
    # ═══ RESUMO ═══
    log.info(f"\n{'='*70}")
    log.info("  RESUMO REENVIO ERRO [1955]")
    log.info(f"{'='*70}")
