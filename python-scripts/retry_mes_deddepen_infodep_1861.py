...
def confirm_token(per_apur: str) -> str:
    return f"RETRY_{per_apur.replace('-', '_')}_DEDDEPEN_INFODEP_1861"
...
def retry_dir(per_apur: str) -> Path:
    return month_dir(per_apur) / "retry_deddepen_infodep_1861"
...
def manifest_path(per_apur: str) -> Path:
    return retry_dir(per_apur) / "manifest_retry_deddepen_infodep_1861.json"
...
def current_error_rows(per_apur: str) -> list[dict[str, Any]]:
...
def prepare_xml(xml_bytes: bytes, per_apur: str, receipt: str, dependentes_message: list[str]) -> tuple[bytes, dict[str, Any]]:
    ...
    ded_dependentes = sorted({child_text(node, "cpfDep") for node in inner.xpath('//*[local-name()="dedDepen"]') if child_text(node, "cpfDep")})
    if not ded_dependentes:
        raise RuntimeError("dedDepen ausente")
    dependentes = sorted(set(ded_dependentes) | set(dependentes_message))
...
def generate_manifest(per_apur: str) -> dict[str, Any]:
    ...
    rows = current_error_rows(per_apur)
    ...
    try:
        for row in rows:
            ...
            try:
                ...
                continue
            out_xml = xml_dir(per_apur) / f"S1210_{per_apur}_{cpf}_deddepen_infodep_1861_unsigned.xml"
    ...
    manifest = {
        ...
        "per_apur": per_apur,
        "regra": "retry 401/1861 de dedDepen: inserir infoDep/cpfDep para CPFs ja usados em dedDepen",
...
def validate_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in manifest.get("targets") or []:
        ...
        ded_depen = [node.xpath('string(./*[local-name()="cpfDep"])') for node in root.xpath('//*[local-name()="dedDepen"]')]
        rows.append(
            {
                "cpf": item["cpf"],
    ...
    wrong = [
        row for row in rows
        if row["perApur"] != manifest["per_apur"] or set(row["dedDepen"]) - set(row["infoDep"]) or row["signature"]
    ]
...
def execute(per_apur: str) -> dict[str, Any]:
    ...
    try:
        ...
        print(f"=> retry dedDepen infoDep {per_apur}: envio_id={envio_id} timeline_mes={mes_id} targets={len(signed)}")
        ...
        envio_base._atualizar_envio(
            ...
            erro=erro_total,
            resumo_extra={
                "rotulo_final": "retry_mes_deddepen_infodep_1861",
                "per_apur": per_apur,
        ...
        (retry_dir(per_apur) / "resultado_retry_deddepen_infodep_1861.json").write_text(
