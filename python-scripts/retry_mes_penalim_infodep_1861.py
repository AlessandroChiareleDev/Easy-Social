...
def confirm_token(per_apur: str) -> str:
    return f"RETRY_{per_apur.replace('-', '_')}_PENALIM_INFODEP_1861"


def retry_dir(per_apur: str) -> Path:
    return ROOT / "relatorio_ana" / "CORRECAO_JAQUE_PLANO_PENSAO_2025" / per_apur / "retry_penalim_infodep_1861"
...
def manifest_path(per_apur: str) -> Path:
    return retry_dir(per_apur) / "manifest_retry_penalim_infodep_1861.json"
...
def current_1861_rows(per_apur: str) -> list[dict[str, Any]]:
...
def add_missing_info_dep(info_ir: etree._Element, dependentes: list[str]) -> list[str]:
    existing = {node.xpath('string(./*[local-name()="cpfDep"])').strip() for node in info_ir.xpath('./*[local-name()="infoDep"]')}
    first_ircr = direct_child(info_ir, "infoIRCR")
    insert_at = info_ir.index(first_ircr) if first_ircr is not None else len(info_ir)
    inserted: list[str] = []
    for offset, cpf_dependente in enumerate(dependentes):
        if cpf_dependente in existing:
            continue
        node = etree.Element(qname("infoDep"))
        sub(node, "cpfDep", cpf_dependente)
        info_ir.insert(insert_at + offset, node)
        inserted.append(cpf_dependente)
    return inserted


def extract_dependentes_from_message(message: str) -> list[str]:
    return sorted({cpf11(match) for match in re.findall(r"dependente\s+(\d{11})\s+inv", message, flags=re.IGNORECASE) if cpf11(match)})


def prepare_xml(xml_bytes: bytes, per_apur: str, receipt: str, dependentes_message: list[str]) -> tuple[bytes, dict[str, Any]]:
    ...
    pen_dependentes = sorted({child_text(node, "cpfDep") for node in inner.xpath('//*[local-name()="penAlim"]') if child_text(node, "cpfDep")})
    if not pen_dependentes:
        raise RuntimeError("penAlim ausente")
    if "00000000000" in pen_dependentes:
        raise RuntimeError("penAlim contem cpfDep zerado")
    dependentes = sorted(set(pen_dependentes) | set(dependentes_message))
    info_ir = ensure_info_ir(inner)
    inserted = add_missing_info_dep(info_ir, dependentes)
    ...
    return output, {
        ...
        "penAlim": pen_dependentes,
        "dependentes_mensagem": dependentes_message,
...
def generate_manifest(per_apur: str) -> dict[str, Any]:
    ...
    rows = current_1861_rows(per_apur)
    ...
    try:
        for row in rows:
            ...
            dependentes_message = extract_dependentes_from_message(str(row.get("erro_mensagem") or ""))
            ...
            try:
                ...
                xml_out, meta = prepare_xml(source_xml, per_apur, str(row["nr_recibo_anterior"]), dependentes_message)
            except Exception as exc:
                conn.rollback()
                skipped.append({**row, "reason": f"{type(exc).__name__}: {exc}"})
                continue
            out_xml = xml_dir(per_apur) / f"S1210_{per_apur}_{cpf}_penalim_infodep_1861_unsigned.xml"
            ...
            targets.append(
                {
                    ...
                    "dependentes": meta["penAlim"],
                    "dependentes_mensagem": meta["dependentes_mensagem"],
                    "validation": meta,
                }
            )
    finally:
    ...
    manifest = {
        ...
        "per_apur": per_apur,
        "regra": "retry 401/1861 de penAlim: inserir infoDep/cpfDep para CPFs ja usados em penAlim",
        "total_current_1861": len(rows),
...
def validate_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in manifest.get("targets") or []:
        ...
        info_dep = [node.xpath('string(./*[local-name()="cpfDep"])') for node in root.xpath('//*[local-name()="infoDep"]')]
        pen_alim = [node.xpath('string(./*[local-name()="cpfDep"])') for node in root.xpath('//*[local-name()="penAlim"]')]
        rows.append(
            {
                "cpf": item["cpf"],
    ...
    wrong = [
        row for row in rows
        if row["perApur"] != manifest["per_apur"] or set(row["penAlim"]) - set(row["infoDep"]) or row["signature"]
    ]
...
def execute(per_apur: str) -> dict[str, Any]:
    ...
    try:
        ...
        print(f"=> retry penAlim infoDep {per_apur}: envio_id={envio_id} timeline_mes={mes_id} targets={len(signed)}")
        ...
        envio_base._atualizar_envio(
            ...
            erro=erro_total,
            resumo_extra={
                "rotulo_final": "retry_mes_penalim_infodep_1861",
                "per_apur": per_apur,
        ...
        (retry_dir(per_apur) / "resultado_retry_penalim_infodep_1861.json").write_text(
