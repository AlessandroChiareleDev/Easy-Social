...
EMPRESA_ID = 3
PER_APUR = "2025-01"
PILOT_CPF = "10477639828"
BATCH_SIZE = 40
POLL_WAITS = [1.0, 1.5, 2.0, 3.0, 4.0] + [5.0] * 55

OUT_ROOT = ROOT / "relatorio_ana" / "OBJETIVA_JAN_MAI_2025" / "xmls_janeiro_70_corrigidos"
MANIFEST_70 = OUT_ROOT / "manifest_70_xmls_corrigidos.json"
MANIFEST_69 = OUT_ROOT / "manifest_69_restantes_apos_piloto.json"
PILOT_RESULT = OUT_ROOT / "retorno_envio_piloto" / PILOT_CPF / "resultado_envio_piloto.json"
RUN_DIR = OUT_ROOT / "retorno_envio_69_restantes"
RESULT_PATH = RUN_DIR / "resultado_envio_69_restantes.json"
        ...
        cursor.execute(
            ...
            (
                ...
                psycopg2.extras.Json(
                    {
                        "rotulo": "objetiva_jan_70_xml_corrigidos",
                        "origem": "relatorio_ana/OBJETIVA_JAN_MAI_2025",
                        "cpf_piloto_ja_enviado": PILOT_CPF,
                        "modo": "producao",
                    }
                ),
            ),
        ...
        cursor.execute(
            ...
            INSERT INTO timeline_envio_item
              (timeline_envio_id, cpf, tipo_evento, status,
               versao_anterior_id, nr_recibo_anterior, nr_recibo_novo,
               xml_enviado_oid, xml_retorno_oid, erro_codigo, erro_mensagem)
            VALUES (%s, %s, 'S-1210', %s, %s, %s, %s, %s, %s, %s, %s)
            RETURNING id
            """,
            (
                envio_id,
...
def insert_success_event(
    ...
    *,
    ...
    dados = {
        "origem": "objetiva_jan_70_xml_corrigidos",
        "acao": item.get("acao"),
        "protocolo": protocolo,
        "codigo": codigo,
        "descricao": descricao,
...
def persist_pilot(conn, envio_id: int, manifest_70: dict, pilot_result: dict) -> dict:
    pilot_item = next(item for item in manifest_70["items"] if item["cpf"] == PILOT_CPF)
    xml_path = ROOT / pilot_item["xml_path"]
    xml_bytes = xml_path.read_bytes()
    info = xml_info(xml_bytes)
    result = pilot_result["resultado_final"]
    envio = pilot_result["envio"]

    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
        old_event_id = find_old_event_id(cursor, PILOT_CPF, pilot_item.get("nr_recibo_anterior"))
    xml_oid = write_lo(conn, xml_bytes)
    raw_consultas = sorted((OUT_ROOT / "retorno_envio_piloto" / PILOT_CPF / "xml_bruto" / "consultas").glob("*.xml"))
    xml_retorno_oid = write_lo(conn, raw_consultas[-1].read_text(encoding="utf-8") if raw_consultas else None)
    item_id = insert_item(
        conn,
        envio_id=envio_id,
        cpf=PILOT_CPF,
        ...
        old_recibo=pilot_item.get("nr_recibo_anterior"),
        xml_enviado_oid=xml_oid,
        nr_recibo_novo=result.get("nr_recibo"),
    ...
    new_event_id = insert_success_event(
        ...
        old_event_id=old_event_id,
        old_recibo=pilot_item.get("nr_recibo_anterior"),
        nr_recibo_novo=result["nr_recibo"],
        codigo=result["codigo_resposta"],
        descricao=result["descricao"],
        protocolo=envio["protocolo"],
        ocorrencias=result.get("ocorrencias") or [],
    )
    return {
        "cpf": PILOT_CPF,
        "status": "sucesso",
...
def send_remaining(conn, envio_id: int, manifest_69: dict, cert: dict) -> list[dict]:
    results = []
    remaining = list(manifest_69["items"])
    for batch_index, start in enumerate(range(0, len(remaining), BATCH_SIZE), start=1):
        ...
        for item in batch_items:
            ...
            old_recibo = item.get("nr_recibo_anterior") or item.get("nr_recibo_rejeitado_459")
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
...
def update_envio_totals(conn, envio_id: int, pilot: dict, results: list[dict]) -> None:
    sucesso = 1 + sum(1 for result in results if result["status"] == "sucesso")
    ...
    protocolos.insert(0, pilot["protocolo"])
    resumo = {
        "finalizado_por": "enviar_69_restantes_persistir_v2.py",
...
def main() -> None:
    ...
    manifest_70 = load_json(MANIFEST_70)
    manifest_69 = load_json(MANIFEST_69)
    pilot_result = load_json(PILOT_RESULT)
    cert = _load_certificado(EMPRESA_ID, None)

    if len(manifest_69["items"]) != 69:
        raise RuntimeError(f"manifest_69 deveria ter 69 items, tem {len(manifest_69['items'])}")
    if any(item["cpf"] == PILOT_CPF for item in manifest_69["items"]):
        raise RuntimeError("manifest_69 contem o CPF piloto; abortando para nao reenviar")

    conn = db.connect(empresa_id=EMPRESA_ID)
    all_results: dict = {}
    try:
        envio_id = create_timeline_envio(conn, total=70)
        print(f"timeline_envio_id={envio_id}", flush=True)
        pilot = persist_pilot(conn, envio_id, manifest_70, pilot_result)
        print(f"piloto persistido cpf={PILOT_CPF} recibo={pilot['nr_recibo_novo']}", flush=True)
        results = send_remaining(conn, envio_id, manifest_69, cert)
        update_envio_totals(conn, envio_id, pilot, results)
        all_results = {
            "ok": True,
            "empresa": "OBJETIVA",
            ...
            "total_sucesso": 1 + sum(1 for result in results if result["status"] == "sucesso"),
