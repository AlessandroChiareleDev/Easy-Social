...
CPF = "00820996777"
RECIBO_INFORMADO = "1.1.0000000039559372733"
OUT_ROOT = ROOT / "relatorio_ana" / "OBJETIVA_JAN_MAI_2025" / "xmls_janeiro_70_corrigidos"
XML_DIR = OUT_ROOT / "04_recibo_459_retificacao_recibo_informado"
RESULT_PATH = OUT_ROOT / "retorno_envio_008_retif_recibo_informado.json"
        ...
        cursor.execute(
            ...
            (
                ...
                psycopg2.extras.Json(
                    {
                        "rotulo": "objetiva_jan_resolve_008_recibo_informado",
                        "cpf": CPF,
                        "recibo_informado": RECIBO_INFORMADO,
                        "motivo": "usuario informou recibo ativo apos 106/459",
                    }
                ),
            ),
        )
...
def load_base_event(conn):
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
        ...
        row = cursor.fetchone()
        if not row:
            raise RuntimeError("evento base local nao encontrado")
        return row
...
def main() -> None:
    ...
    try:
        ...
        xml_path = XML_DIR / f"S1210_2025-01_{CPF}_retificacao_recibo_informado_assinado.xml"
        ...
        returned, attempts = poll_batch(protocolo, {info["id_evento"]}, cert)
