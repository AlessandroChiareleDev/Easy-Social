-- =============================================================================
-- Easy Social — Migration Inicial para Supabase
-- Unifica easy_social_db + easy_social_master em um único banco
-- =============================================================================
-- As tabelas do antigo easy_social_master (usuarios, empresas, usuario_empresa)
-- ficam no schema "master" para não conflitar com auth.users do Supabase.
-- As tabelas do easy_social_db ficam no schema "public" (padrão).
-- =============================================================================

-- =====================
-- SCHEMA: master (auth/multi-tenant)
-- =====================
CREATE SCHEMA IF NOT EXISTS master;

-- Tabela de empresas
CREATE TABLE master.empresas (
...
-- Tabela de perfis (substituindo usuarios — auth fica no Supabase Auth)
-- Vincula auth.users.id ao perfil do app
CREATE TABLE master.perfis (
...
CREATE TABLE master.usuario_empresa (
    id serial PRIMARY KEY,
    usuario_id uuid REFERENCES master.perfis(id) ON DELETE CASCADE,
    empresa_id integer REFERENCES master.empresas(id) ON DELETE CASCADE,
    role_emp varchar(20) DEFAULT 'operador' CHECK (role_emp IN ('admin', 'operador')),
    criado_em timestamptz DEFAULT now(),
    UNIQUE(usuario_id, empresa_id)
);

-- Naturezas no schema master (cópia de referência global)
CREATE TABLE master.naturezas_esocial (
    id serial PRIMARY KEY,
...
-- =====================
-- SCHEMA: public (dados da empresa)
-- =====================

-- Upload tracking (Excel DIRF)
...
-- Análise de Natureza (DIRF sheet)
...
-- Análise de Natureza (cópia corrigida)
...
-- Auditoria de alterações de natureza
...
-- Cruzamento EB (source-of-truth para S-1010)
...
-- eSocial Envios (histórico S-1010)
...
-- eSocial Tabela 3 Natureza (importação oficial)
...
-- FK que depende de tabela_eb (criada depois)
...
-- FK cruzamento_resultado → cruzamento_uploads
ALTER TABLE public.cruzamento_resultado
    ADD CONSTRAINT cruzamento_resultado_cruzamento_upload_id_fkey
    FOREIGN KEY (cruzamento_upload_id) REFERENCES public.cruzamento_uploads(id) ON DELETE CASCADE;
